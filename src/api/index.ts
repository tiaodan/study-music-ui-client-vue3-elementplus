...
const HttpManager = {
  ...
  // 通过地区和性别对歌手分类
  getSingerOfAreaAndSex: (area, sex) => get(`singer/area/sex/detail?area=${area}&sex=${sex}`),
  // 通过地区、性别和字母筛选歌手
  getSingerByCondition: (area, sex, initial) => get(`singer/condition?area=${area}&sex=${sex}&initial=${initial}`),
